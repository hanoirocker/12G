namespace TwelveG.InteractableObjects
{
    ...
    public class CleanBirdsHandler : MonoBehaviour, IInteractable
    {
        ...
        [SerializeField] private InteractableFallbackTextsSO interactableFallbackTextsSO;
        ...
        public string GetFallBackText(string currentLanguage)
        {
            if (!playerItems.Contains(ItemType.EmptyTrashBag.ToString()) && playerItems.Contains(ItemType.Broom.ToString()))
            {
                ...
                return Utils.TextFunctions.RetrieveInteractableFallbackText(currentLanguage, 1, interactableFallbackTextsSO);
            }
            if (playerItems.Contains(ItemType.EmptyTrashBag.ToString()) && !playerItems.Contains(ItemType.Broom.ToString()))
            {
                //"Aun me falta la escoba";
                return Utils.TextFunctions.RetrieveInteractableFallbackText(currentLanguage, 2, interactableFallbackTextsSO);
            }
            else
            {
               //"La escoba de la cocina y una bolsa me vendrian bien para esto...";
               return Utils.TextFunctions.RetrieveInteractableFallbackText(currentLanguage, 0, interactableFallbackTextsSO);
            }
        }
    }
}